...
#                     res.push s[ s.length - 1 ]
        ...
        #
        @app_data.focus.set @cm.process_id
    ...
    _update_borders: ->        
        ...
        @divCanvas.style.width  = @p_max[ 0 ] - @p_min[ 0 ]
        @divCanvas.style.height = @p_max[ 1 ] - @p_min[ 1 ] - 30
    ...
    _add_transform_node: ( evt ) =>
        for m in @app_data.modules when m instanceof TreeAppModule_Transform
            m.actions[ 1 ].fun evt, @app_data._processes[ 0 ]
