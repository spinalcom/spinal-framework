...
class IcoBar extends View
    ...
    init: ->
        ...
        context_modules.set @app_data.modules
...
#         if @app_data.selected_tree_items.has_been_directly_modified  
        ...
        for path in @app_data.selected_tree_items
            ...
            #if item instanceof GmshItem
            #    alert item._context_modules
            if item._context_modules?
              for module in item._context_modules
                  context_modules.push module if not (context_modules.contains(module))
