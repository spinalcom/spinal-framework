...
class LineTheme extends Model
    constructor: ( color = new Color( 255, 255, 255, 255 ), width = 2 ) ->
        super()
    ...
    beg_ctx: ( info ) ->
        if info.ctx_type == "2d"
            ...
            @ps.colorUniform = gl.getUniformLocation @ps, "u_color"
            @ps.wUniform = gl.getUniformLocation @ps, "w"
            @ps.hUniform = gl.getUniformLocation @ps, "h"
    ...
    draw_straight_proj: ( info, p0, p1 ) ->
        if info.ctx_type == "2d"
            ...
            gl.lineWidth @width.get()
            gl.uniform4f @ps.colorUniform, @color.r.get()/255, @color.g.get()/255, @color.b.get()/255, @color.a.get()/255
            gl.uniform1f @ps.wUniform, info.w
            gl.uniform1f @ps.hUniform, info.h
