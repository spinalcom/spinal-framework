...
class Element_Line extends Element_WithIndices
    ...
    draw_gl: ( info, mesh, points, is_a_sub, theme = info.theme.lines ) ->
        ...
        if wf or not is_a_sub
            ...
            if @indices.length
                for i in [ 0 ... @indices.length - 1 ]
                    theme.draw_straight_proj info, proj[ @indices[ i ].get() ], proj[ @indices[ i + 1 ].get() ]
        ...
        if @indices.length >= 2
            if inversion
                for i in [ @indices.length - 2 .. 0 ]
                    mesh._theme.lines.contour_straight_proj info, proj[ @indices[ i + 1 ].get() ], proj[ @indices[ i ].get() ], beg and i == 0
            else
                for i in [ 0 ... @indices.length - 1 ]
                    mesh._theme.lines.contour_straight_proj info, proj[ @indices[ i ].get() ], proj[ @indices[ i + 1 ].get() ], beg and i == 0
