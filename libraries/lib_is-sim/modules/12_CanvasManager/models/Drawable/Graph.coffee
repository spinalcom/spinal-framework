...
#         @origin       = [ info.padding, info.h - info.padding ]
    ...
    add_shadow: ( info ) ->
        info.ctx_2d().shadowOffsetX = @line_width.get()
        info.ctx_2d().shadowOffsetY = @line_width.get()
        info.ctx_2d().shadowBlur    = @line_width.get()
        info.ctx_2d().shadowColor   =  "#3a3a3a"
    ...
    hide_outside_values: ( info ) ->     
        ...
        info.ctx_2d().clearRect info.padding * 0.5, info.h - info.padding / 2,info.w, info.padding * 0.5
    ...
    draw_movable_highlight_values: ( info ) ->
        ...
        for p, i in @points
            if @points[ i ] in @_pre_sele
                ...
                info.ctx_2d().font = @font_size.get() * 2 + "px Arial"
        ...
        #         
                ...
                info.ctx_2d().fillStyle = @font_color.to_rgba()
                info.ctx_2d().fillText text , pos[ 0 ] + padding_left + decal_left, pos[ 1 ] + padding_top + decal_top
    ...
    draw_highlight_values: ( info ) ->
        ...
        for p, i in @points
            if @points[ i ] in @_pre_sele
                ...
                info.ctx_2d().font = @font_size.get() * 2 + "px Arial"
                info.ctx_2d().fillText highlighted_point[ 0 ] + ", " + highlighted_point[ 1 ] ,  info.w - padding , 20
    
    
    draw_line: ( info, orig, proj ) ->
        #draw real line
        info.ctx_2d().beginPath()
        info.ctx_2d().strokeStyle = @line_color.to_rgba()
        if @line_width.get() <= 1
            info.ctx_2d().lineWidth = 1.000001 #fix a chromium bug
        else
            info.ctx_2d().lineWidth = @line_width.get()
        for p, i in proj
    ...
    draw_marker_dot: ( info, orig, proj ) ->
        for p, i in proj
            if @points[ i ] in @_pre_sele
                info.ctx_2d().fillStyle = @sel_item_color.to_rgba()
            else
                info.ctx_2d().fillStyle = @legend[ i ] or @marker_color.to_rgba()
            info.ctx_2d().beginPath()
            info.ctx_2d().arc p[ 0 ], p[ 1 ], @marker_size.get() * 0.5, 0, Math.PI * 2, true
            info.ctx_2d().fill()
        info.ctx_2d().closePath()
    ...
    draw_marker_cross: ( info, orig, proj ) ->
        for p, i in proj
            if @points[ i ] in @_pre_sele
                info.ctx_2d().strokeStyle = @sel_item_color.to_rgba()
            else
                info.ctx_2d().strokeStyle = @legend[ i ] or @marker_color.to_rgba()
                
            info.ctx_2d().beginPath()
            info.ctx_2d().moveTo p[ 0 ] - @marker_size.get() * 0.5, p[ 1 ] + @marker_size.get() * 0.5
            info.ctx_2d().lineTo p[ 0 ] + @marker_size.get() * 0.5, p[ 1 ] - @marker_size.get() * 0.5
            info.ctx_2d().moveTo p[ 0 ] + @marker_size.get() * 0.5, p[ 1 ] + @marker_size.get() * 0.5
            info.ctx_2d().lineTo p[ 0 ] - @marker_size.get() * 0.5, p[ 1 ] - @marker_size.get() * 0.5
            info.ctx_2d().stroke()
        info.ctx_2d().closePath()
    ...
    draw_marker_square: ( info, orig, proj ) ->
        for p, i in proj
            if @points[ i ] in @_pre_sele
                ...
                info.ctx_2d().fillStyle = @legend[ i ] or @marker_color.to_rgba()
            info.ctx_2d().beginPath()
            info.ctx_2d().fillRect p[ 0 ] - @marker_size.get() * 0.5 , p[ 1 ] - @marker_size.get() * 0.5 , @marker_size.get(), @marker_size.get()
        info.ctx_2d().closePath()
    ...
    draw_marker_diamond: ( info, orig, proj ) ->
        for p, i in proj
            if @points[ i ] in @_pre_sele
                ...
                info.ctx_2d().fillStyle = @legend[ i ] or @marker_color.to_rgba()
            info.ctx_2d().beginPath()
            info.ctx_2d().moveTo p[ 0 ], p[ 1 ] - @marker_size.get()
            info.ctx_2d().lineTo p[ 0 ] + @marker_size.get() * 0.5, p[ 1 ]
            info.ctx_2d().lineTo p[ 0 ], p[ 1 ] + @marker_size.get()
            info.ctx_2d().lineTo p[ 0 ] - + @marker_size.get() * 0.5, p[ 1 ]
            info.ctx_2d().fill()
        info.ctx_2d().closePath()
    ...
    draw_marker_bar: ( info, orig, proj ) ->
        for p, i in proj
            ...
            info.ctx_2d().beginPath()
            info.ctx_2d().fillRect p[ 0 ], p[ 1 ], @marker_size.get(), height
        info.ctx_2d().closePath()
    ...
    draw_axis: ( info ) ->
        ...
        info.ctx_2d().beginPath()
        info.ctx_2d().lineWidth = @axis_width
        info.ctx_2d().strokeStyle = @font_color.to_rgba()
        info.ctx_2d().fillStyle = @font_color.to_rgba()
        info.ctx_2d().font = @font_size.get() + "px Arial"
        ...
        if @x_axis.get() != ""
            info.ctx_2d().textAlign = "left"
            info.ctx_2d().fillText @x_axis.get(), orig[ 0 ] + width_axis + decal_txt, orig[ 1 ] + 2
        ...
        if @y_axis.get() != ""
            ...
            info.ctx_2d().fillText @y_axis.get(),  orig[ 0 ] - 2, height_axis - decal_txt
        info.ctx_2d().moveTo orig[ 0 ], orig[ 1 ]
    ...
    draw_legend: ( info ) ->
        ...
        info.ctx_2d().beginPath()
        info.ctx_2d().fillStyle = @font_color.to_rgba()
        info.ctx_2d().font = @font_size.get() * 0.8 + "px Arial"
...
#             info.ctx_2d().fillText val.toPrecision( size ), pos, orig[ 1 ] + x_padding_txt
...
#         console.log res
    ...
    draw_grid: ( info ) ->
        ...
        info.ctx_2d().beginPath()
        info.ctx_2d().strokeStyle = @grid_color.to_rgba()
